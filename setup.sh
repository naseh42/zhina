...
install_prerequisites() {
    ...
    apt-get install -y \
        ...
        libpq-dev || error "خطا در نصب پکیج‌ها"
...
setup_python() {
    ...
    pip install \
        ...
        python-jose[cryptography]==3.3.0 || error "خطا در نصب نیازمندی‌ها"
    deactivate
...
create_admin_user() {
    info "ایجاد کاربر ادمین در دیتابیس..."
...
conn.close()
EOF
    "$INSTALL_DIR/venv/bin/python" /tmp/create_admin.py || error "خطا در ایجاد کاربر ادمین"
    rm /tmp/create_admin.py
    success "کاربر ادمین با موفقیت ایجاد شد"
}
...
show_installation_info() {
    ...
    echo -e "\n${YELLOW}مشخصات دسترسی به پنل:${NC}"
    echo -e "• آدرس پنل: ${GREEN}http://${PANEL_DOMAIN}:${PANEL_PORT}${NC}"
    ...
    echo -e "\n${YELLOW}دستورات مدیریت سرویس‌ها:${NC}"
